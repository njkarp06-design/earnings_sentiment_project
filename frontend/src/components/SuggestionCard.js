...
export default function SuggestionCard({ item, onInspect }) {
  ...
  return (
    <div
      className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden hover:border-slate-500 transition-colors cursor-pointer"
      onClick={() => onInspect?.(item)}
    >
      {/* ── Header ──────────────────────────────────────────── */}
      <div className="flex items-start justify-between gap-2 px-4 pt-3 pb-2">
        <div className="min-w-0">
          <span className="font-semibold text-blue-400 text-sm">{item.ticker}</span>
          {item.company_name && (
            <span className="ml-1.5 text-slate-500 text-xs truncate">{item.company_name}</span>
          )}
        </div>
        {item.sector && (
          <span className="text-[10px] bg-slate-700 text-slate-400 px-2 py-0.5 rounded-full shrink-0">
            {item.sector}
          </span>
        )}
      </div>
