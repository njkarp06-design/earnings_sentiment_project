...
function accentBorder(score, hasData) {
  if (!hasData) return 'border-t-slate-300';
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={clsx(
            'w-full max-w-2xl bg-white border border-slate-200 border-t-[3px] rounded-2xl overflow-hidden shadow-[0_8px_40px_rgba(0,0,0,0.12)]',
        ...
        >
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                {activeItem.model_used && (
                  <span className="text-[10px] text-slate-600 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded-full shrink-0">
                    {activeItem.model_used.replace('claude-', '')}
                  </span>
                )}
              </div>
              {activeItem.company_name && (
                <p className="text-slate-500 text-sm mt-0.5">{activeItem.company_name}</p>
              )}
              <p className="text-slate-400 text-xs mt-1">{fmtDate(activeItem.call_date)}</p>
            </div>
            <button
              onClick={onClose}
          ...
          {hasData ? (
            <>
              ...
              {/* ── Returns ──────────────────────────────────────────── */}
              <div className="flex gap-8 px-6 py-4 border-t border-slate-200">
              ...
              {history.length >= 1 ? (
                ...
                <div className="flex items-start gap-3 px-6 py-4 border-t border-slate-200">
                  <div className="w-4 h-4 rounded-full border-2 border-blue-600 border-t-transparent animate-spin shrink-0 mt-0.5" />
                  <div>
                    <p className="text-sm font-medium text-slate-800">Building call history</p>
              ...
              <div className="flex flex-col gap-3 px-6 py-4 border-t border-slate-200">
                <div>
            ...
            <div className="px-6 py-6 border-t border-slate-200">
              {fetchState === 'loading' ? (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-slate-800">Fetching from SEC EDGAR…</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-slate-800">No recent earnings calls found</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-slate-800">No earnings data yet</p>
                    <p className="text-xs text-slate-500 mt-1">
                      {item.ticker} is in our universe but hasn&apos;t been scored yet.
                    </p>
                    {item.sector && (
                      <span className="inline-block mt-2 text-[10px] bg-slate-100 border border-slate-200 text-slate-600 px-2.5 py-0.5 rounded-full">
                        {item.sector}
                      </span>
                    )}
                    {isLoggedIn && (
                      <button
                        onClick={handleFetch}
                        className="mt-3 flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-semibold bg-blue-700 hover:bg-blue-600 text-white transition-colors"
                      >
          ...
          {/* ── Footer CTAs ─────────────────────────────────────────── */}
          <div className="flex flex-col gap-2 px-6 py-4 border-t border-slate-200 bg-slate-50">
              ...
              <div className="flex items-center gap-2">
                {hasData && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium bg-slate-100 hover:bg-slate-200 border border-slate-200 text-slate-600 hover:text-slate-900 transition-colors"
                  >
                    <SparkleIcon />
                    Deep Analysis
                  </button>
                ...
                {isLoggedIn ? (
                  ...
                  <Link
                    href="/login"
                    onClick={onClose}
                    className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-slate-100 border border-slate-200 text-slate-600 hover:bg-slate-200 transition-colors"
                  >
            ...
            {portfolioError && (
              <p className="text-xs text-red-600 text-right">{portfolioError}</p>
            )}
          </div>
        </div>
      </div>
