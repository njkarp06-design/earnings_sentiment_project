...
function winColor(rate) {
  ...
  return rate >= 0.5 ? 'text-emerald-400' : 'text-red-400';
}
...
function ChartTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ color: '#475569', marginBottom: 4, fontSize: 11 }}>Day {label}</div>
      ...
      <ComposedChart data={avgPath} margin={{ top: 8, right: 16, left: -4, bottom: 0 }}>
        ...
        {showBand && (
          <>
            ...
            <Area dataKey="bandHeight" fill="#06b6d4" fillOpacity={0.08} stroke="none" stackId="band"
              type="monotone" isAnimationActive={false} activeDot={false} connectNulls legendType="none" />
          </>
        )}
...
function SectorCard({ sector, selected, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className={clsx(
        'w-full text-left bg-slate-900 border rounded-xl p-5 transition-all duration-200',
        'hover:border-slate-700 hover:shadow-card-hover',
        selected
          ? 'border-cyan-500/40 shadow-[0_0_0_1px_rgba(6,182,212,0.2)]'
          : 'border-slate-800',
      )}
    >
      <div className="flex items-start justify-between gap-2 mb-3">
        <h3 className="font-semibold text-slate-200 text-sm leading-snug">{sector.sector}</h3>
        <span className={clsx(
      ...
      <div className="text-[10px] text-slate-600 uppercase tracking-widest mb-4">Avg 7d return</div>

      <div className="flex items-center gap-2 text-[10px] text-slate-600 font-mono border-t border-slate-800/60 pt-3">
        ...
        <span className="text-slate-700 ml-auto">{sector.call_count} calls · {sector.company_count} co.</span>
      </div>
    </button>
  );
}
...
function SectorDetail({ sector, detail, loading, onCompanyClick }) {
  return (
    <div className="bg-slate-900 border border-cyan-500/20 rounded-xl overflow-hidden mb-6">

      {/* Header */}
      <div className="px-6 pt-5 pb-4 border-b border-slate-800">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h2 className="text-lg font-bold text-slate-100 tracking-tight">{sector}</h2>
      ...
      {loading ? (
        <div className="flex items-center justify-center h-48 text-slate-600 animate-pulse font-mono text-sm">
          Loading…
        </div>
      ) : detail ? (
        <>
            ...
            <div className="flex items-center justify-between mb-3 px-2">
              ...
              <div className="flex items-center gap-3 text-[10px] text-slate-600 font-mono">
                <span className="flex items-center gap-1.5">
                  <span className="w-4 border-t-[2px] border-cyan-500 inline-block" />
                  Sector avg
                </span>
                <span className="flex items-center gap-1.5">
          ...
          {detail.companies.length > 0 && (
            <div className="border-t border-slate-800 mt-2">
              <div className="px-6 py-3 text-[10px] text-slate-600 uppercase tracking-widest">
                Company Rankings
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-800">
                      <th className="px-6 py-2 text-left text-[10px] text-slate-600 uppercase tracking-widest w-8">#</th>
                      <th className="px-4 py-2 text-left text-[10px] text-slate-600 uppercase tracking-widest">Company</th>
                      <th className="px-4 py-2 text-center text-[10px] text-slate-600 uppercase tracking-widest">Calls</th>
                      <th className="px-4 py-2 text-center text-[10px] text-slate-600 uppercase tracking-widest">Avg 1d</th>
                      <th className="px-4 py-2 text-center text-[10px] text-slate-600 uppercase tracking-widest">Avg 3d</th>
                      <th className="px-4 py-2 text-center text-[10px] text-slate-600 uppercase tracking-widest">Avg 7d</th>
                      <th className="px-4 py-2 text-center text-[10px] text-slate-600 uppercase tracking-widest">Win Rate</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800/50">
                    {detail.companies.map((co, i) => (
                      ...
                      >
                        <td className="px-6 py-2.5 text-slate-600 font-mono text-xs">{i + 1}</td>
                        <td className="px-4 py-2.5">
                          <span className="font-mono font-bold text-cyan-400 text-sm">{co.ticker}</span>
                          {co.company_name && (
                            <span className="text-slate-600 text-xs ml-2">{co.company_name}</span>
                          )}
                        </td>
...
function Stat({ label, value, color }) {
  return (
    <div className="text-right">
      ...
      <div className={clsx('text-sm font-mono font-semibold tabular-nums', color ?? 'text-slate-200')}>{value}</div>
    </div>
  );
}
...
export default function SectorsPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-100 tracking-tight">Sector Pulse</h1>
      ...
      {!loading && !error && sectors.length === 0 && (
        <p className="text-center py-16 text-slate-600">
          No sector data yet — returns must be correlated for entries to appear.
        </p>
      )}
