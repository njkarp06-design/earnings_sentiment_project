...
# ── Self-hosted Kafka (KRaft, single broker) ──────────────────────────────────
...
# ── One-shot: create Kafka topics ─────────────────────────────────────────────
# Triggered by the CI/CD pipeline via `aws ecs run-task` after Kafka is ready.
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        { name = "KAFKA_BOOTSTRAP_SERVERS", value = "kafka.esp.local:9092" },
      ]
      logConfiguration = {
        logDriver = "awslogs"
        options = {
