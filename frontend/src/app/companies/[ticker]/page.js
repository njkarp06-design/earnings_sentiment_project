...
function StatChip({ label, value, scoreColored, positive }) {
  let valueClass = 'text-slate-200 font-mono tabular-nums';
  if (scoreColored && value != null) valueClass = clsx('font-mono font-semibold tabular-nums', scoreTextColor(value));
  if (positive === true)  valueClass = 'text-emerald-400 font-mono font-semibold tabular-nums';
  if (positive === false) valueClass = 'text-red-400 font-mono font-semibold tabular-nums';
  return (
    <div>
...
export default function CompanyPage({ params }) {
  ...
  if (loading) {
    return (
      <div className="text-center py-20 text-slate-600 animate-pulse font-mono">
        Loading {ticker}…
      </div>
    );
  }
  ...
  if (history.length === 0) {
    return (
      <div>
        {/* Back link */}
        <Link href="/" className="inline-flex items-center gap-1.5 text-xs text-slate-600 hover:text-slate-400 transition-colors mb-6 group">
        ...
        <div className="py-10 max-w-md mx-auto text-center">
          <h1 className="text-2xl font-bold font-mono text-slate-100 tracking-tight mb-1">{ticker}</h1>
          {companyName !== ticker && (
            <p className="text-slate-500 text-sm mb-4">{companyName}</p>
          )}
          {sector && (
            <span className="inline-block mb-6 text-xs bg-slate-800 border border-slate-700/50 text-slate-500 px-3 py-1 rounded-full">
              {sector}
            </span>
          )}
  ...
  return (
    <div>
      {/* ── Back link ────────────────────────────────────────────── */}
      <Link href="/" className="inline-flex items-center gap-1.5 text-xs text-slate-600 hover:text-slate-400 transition-colors mb-5 group">
        <svg className="w-3 h-3 group-hover:-translate-x-0.5 transition-transform" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
        </svg>
        Feed
      ...
      <div className="flex items-start justify-between gap-4 mb-2">
        <div>
          <div className="flex items-center gap-3 mb-1 flex-wrap">
            <h1 className="text-3xl font-bold font-mono text-slate-100 tracking-tight">{ticker}</h1>
            {sector && (
              <span className="text-xs bg-slate-800 border border-slate-700/50 text-slate-500 px-2.5 py-1 rounded-full">
                {sector}
              </span>
            )}
          </div>
      ...
      {/* ── Quick stats strip ────────────────────────────────────── */}
      <div className="flex items-center gap-8 py-4 mb-6 border-y border-slate-800">
...
function fmtReturn(val) {
  ...
  const pos = val >= 0;
  return (
    <span className={`font-mono tabular-nums ${pos ? 'text-emerald-400' : 'text-red-400'}`}>
      {pos ? '+' : ''}{val.toFixed(2)}%
    </span>
  );
}
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 mb-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-[10px] text-slate-700 font-mono">{accuracy.total} calls with returns</span>
      </div>
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          {item.model_used && (
            <span className="text-[10px] text-slate-600 bg-slate-800 border border-slate-700/50 px-2 py-0.5 rounded-full">
              {item.model_used.replace('claude-', '')}
            </span>
          )}
          <button
            onClick={() => setInspecting(true)}
            className="flex items-center gap-1.5 text-[11px] text-slate-500 hover:text-cyan-400 bg-slate-800 border border-slate-700/50 hover:border-cyan-500/30 px-2.5 py-1 rounded-full transition-colors"
          >
      ...
      {item.key_phrases?.length > 0 && (
        <div className="mb-4">
          <div className="text-[10px] text-slate-600 uppercase tracking-widest mb-2">Key Phrases</div>
